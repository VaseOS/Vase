...
This version tries to implement a **guided-first** approach (no JSON files, scripts, etc) instead sane-defaults and more advanced features straight from the TUI menu.
...
Login the ugly SDDM screen (or another Login-Manager) using user you created earlier.

---
Open the apps launcher > type `Konsole` ro `Console` ro whatever terminal included in DE.
...
And post install logs inside: `vase_os/kaes_arch`

We also have 2 helper scripts for debugging install logs: 
```
./main -xlu
```
This will return a URL with a short ex: `https://0x0.st/KjFo.log` useful to share logs or watch them from another machine.

```
./main -xl
```
This just shows you the essential prints of logs wihtout uploading.
